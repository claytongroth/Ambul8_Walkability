...
def receive_coords(lat, lng):
    ...
    statsJson = ((amb.generateWS(point))[3])
    response = make_response(jsonify(isochroneGJ=isochroneGJ, WS=WS, amenityCount=json.loads(amenityCount), amenityGJ=json.loads(amenityGJ), point=point, statsJson=statsJson))
